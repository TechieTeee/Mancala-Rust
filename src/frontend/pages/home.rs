        ...
        <div class="instructions-modal">
            <div class="instructions-content">
                ...
                <div class="instructions-text">
                    ...
                    <p><strong>{"Setup:"}</strong> {" Each player controls 6 pits with 4 stones each. Player 1 controls the bottom row, Player 2 controls the top row. The large pits on each side are your 'stores' where you collect captured stones."}</p>
...
pub fn home() -> Html {
        ...
        Callback::from(move |pit_index: usize| {
            ...
            if new_game.make_move(pit_index) {
                ...
                if new_game.game_over {
                    audio.play_sound(SoundEvent::GameOver);
                } else if new_game.current_player == old_game.current_player {
                    audio.play_sound(SoundEvent::ExtraTurn);
                } else {
                    // Check if a capture occurred by comparing stores
                    let old_store = if old_game.current_player == 0 {
                        old_game.board[6]
                    } else {
                        old_game.board[13]
                    };
                    let new_store = if old_game.current_player == 0 {
                        new_game.board[6]
                    } else {
                        new_game.board[13]
                    };
                    // A capture adds more than 1 stone to the store beyond normal distribution
                    let stones_moved = old_game.board[pit_index];
                    let expected_store_gain = {
                        // Count how many times we'd pass through our store
                        let mut count = 0u8;
                        let opponent_store = if old_game.current_player == 0 { 13 } else { 6 };
                        let own_store = if old_game.current_player == 0 { 6 } else { 13 };
                        let mut idx = pit_index;
                        let mut remaining = stones_moved;
                        while remaining > 0 {
                            idx = (idx + 1) % 14;
                            if idx == opponent_store { continue; }
                            if idx == own_store { count += 1; }
                            remaining -= 1;
                        }
                        count
                    };
                    if new_store - old_store > expected_store_gain {
                        audio.play_sound(SoundEvent::Capture);
                    }
                }
                game.set(new_game);
            }
    ...
    let reset_game = {
        ...
        Callback::from(move |_: MouseEvent| {
            audio.play_sound(SoundEvent::NewGame);
    ...
    let player_class = if game.game_over {
        "game-over"
    ...
    html! {
        <div class={classes!("container", player_class)}>
            <h1>{ "Mancala" }</h1>
